...
/** Every query starts from `votes`: comparisons joined to their session, optionally without legacy rows. */
const VOTES = `WITH votes AS (
  ...
  WHERE $1::boolean OR s.participant_id IS NOT NULL
), shown AS (
...
const run = (body: string) => sql.query(`${VOTES} ${body}`, [includeLegacy]) as Promise<Row[]>;
...
const SECTIONS: Record<string, { title: string; note?: string; query: () => Promise<Row[]> }> = {
  overview: {
    ...
    query: () => run(`SELECT
      ...
      (SELECT count(*) FROM science_sessions s WHERE s.completed_at IS NOT NULL AND ($1::boolean OR s.participant_id IS NOT NULL)) AS completed,
...
if (flags.json) {
  console.log(JSON.stringify({ takenAt: new Date().toISOString(), env: flags.env, database: describeDatabase(), includeLegacy, ...results }, null, 2));
} else {
  console.log(`Land or Bounce · human vs fly · ${new Date().toISOString().replace('T', ' ').slice(0, 16)} UTC · ${describeDatabase()} (${flags.env})${includeLegacy ? ' · including legacy sessions' : ''}`);
