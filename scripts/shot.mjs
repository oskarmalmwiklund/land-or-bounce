...
const [url, prefix, waits = '3000', width = '1440', height = '960', clickBeforeLast = ''] = process.argv.slice(2);
...
let t = 0;
const list = waits.split(',').map(Number);
for (const [i, w] of list.entries()) {
  await new Promise((r) => setTimeout(r, w - t)); t = w;
  if (clickBeforeLast && i === list.length - 1) { await page.click(clickBeforeLast); await new Promise((r) => setTimeout(r, 1200)); }
