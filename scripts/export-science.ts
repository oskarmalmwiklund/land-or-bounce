...
const [captures, comparisons] = await Promise.all([
  sql`SELECT c.id AS capture_id, s.canonical_url, s.hostname, c.screenshot_url, c.screenshot_hash, c.viewport_width, c.viewport_height, c.fold, c.captured_at, f.total_score, f.notice, f.landing_spot, f.calm, f.balance, f.colour, f.raw_metrics, f.simulator_version, f.scoring_version FROM science_captures c JOIN science_sites s ON s.id = c.site_id JOIN science_fly_runs f ON f.capture_id = c.id WHERE c.consented_for_science = true ORDER BY c.captured_at`,
